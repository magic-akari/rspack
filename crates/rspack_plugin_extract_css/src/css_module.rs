...
use rspack_core::{
  impl_build_info_meta, impl_source_map_config, AsyncDependenciesBlockId, BuildContext, BuildInfo,
  BuildMeta, BuildResult, CodeGenerationResult, Compilation, CompilerOptions, DependenciesBlock,
  DependencyId, DependencyType, Module, ModuleFactory, ModuleFactoryCreateData,
...
impl Identifiable for CssModule {
  fn identifier(&self) -> rspack_identifier::Identifier {
    format!(
      "css|{}|{}|{}|{}}}",
      self.identifier, self.identifier_index, self.supports, self.media,
    )
    .into()
  }
}
