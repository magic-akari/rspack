...
use rspack_core::{
  ...
  CompilationArgs, CompilationParams, CompilerOptions, Filename, Module, ModuleIdentifier,
  ModuleType, PathData, Plugin, PluginCompilationHookOutput, PluginContext,
  PluginRenderManifestHookOutput, PluginRuntimeRequirementsInTreeOutput, RenderManifestArgs,
  RenderManifestEntry, RuntimeGlobals, RuntimeRequirementsInTreeArgs, SourceType,
};
...
pub static MODULE_TYPE: Lazy<ModuleType> =
  Lazy::new(|| ModuleType::Custom(MODULE_TYPE_STR.clone()));
pub static SOURCE_TYPE: Lazy<[SourceType; 1]> =
  Lazy::new(|| [SourceType::Custom(MODULE_TYPE_STR.clone())]);
...
struct CssOrderConflicts {
  ...
  // (module, failed chunkGroups, fullfilled chunkGroups)
  ...
  fn sort_modules<'comp>(
    ...
    let mut modules_by_chunk_group = groups
      ...
      .map(|chunk_group| {
        ...
        for (i, (module, _)) in sorted_module.iter().enumerate() {
          ...
          let reasons = module_deps_reasons.get_mut(module).unwrap();
    ...
    while used_modules.len() < modules.len() {
      ...
      for list in &mut modules_by_chunk_group {
        ...
        while list.len() > 0 && used_modules.contains(&list.last().unwrap().0) {
          list.pop();
        }

        // skip empty lists
        if !list.is_empty() {
          let module = list.last().unwrap().0;
          let deps = module_dependencies.get(&module).unwrap();
          ...
          if best_match_deps.is_none()
            || best_match_deps.as_ref().unwrap().len() > failed_deps.len()
          {
      ...
      if !success {
        ...
        // and emit a warning
        let mut best_match = best_match.unwrap();
        let best_match_deps = best_match_deps.unwrap();
        let fallback_module = best_match.pop().unwrap();
        if !self.options.ignore_order {
          let reasons = module_deps_reasons.get(&fallback_module).unwrap();
    ...
    for module in used_modules {
      ...
      if starts_with_at_import {
        ...
        let undo_path = get_undo_path(
          &filename,
          compilation.options.output.path.to_str().unwrap(),
          false,
        );
...
despite it was not able to fulfill desired ordering with these modules:\n{}",
              chunk
                ...
                .as_ref()
                .map(|s| { s.as_str() })
                .unwrap_or(chunk.id.as_ref().map(|s| s.as_str()).unwrap_or_default()),
  ...
  for part in PATH_SEP.split(filename) {
    if part == ".." {
      if depth > -1 {
        ...
        append = format!("{}/{append}", output_path[pos.unwrap() + 1..].to_string());
