...
async fn loader_test(actual: impl AsRef<Path>, expected: impl AsRef<Path>) {
  ...
  let (result, _) = run_loaders(
    ...
    &[],
    CompilerContext {
      options: std::sync::Arc::new(compiler_options),
      resolver_factory: Default::default(),
      module: "".into(),
      module_context: None,
      module_source_map_kind: SourceMapKind::SourceMap,
      compilation_queue_handler: Default::default(),
      compilation_plugin_driver: plugin_driver,
      compilation_cache: Arc::new(rspack_core::cache::Cache::new(compiler_options)),
    },
  )
  .await
  .expect("TODO:")
