...
pub struct CompilerContext {
  ...
  pub compilation_queue_handler: Option<QueueHandler>,
  pub compilation_plugin_driver: SharedPluginDriver,
  pub compilation_cache: Arc<Cache>,
}
...
impl CompilerContext {
  pub async fn import_module(
    ...
    if self.compilation_queue_handler.is_none() {
    ...
    Compilation::import_module_impl(
      self.compilation_queue_handler.clone().expect("unreachable"),
      self.resolver_factory.clone(),
      self.options.clone(),
      self.compilation_plugin_driver.clone(),
